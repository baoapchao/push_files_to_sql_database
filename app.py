...
label1 = tk.Label(text= 'Choose file to import to DB')
label1.grid(row=1, column=1)

label2 = tk.Label(text= 'Input Connection String')
label2.grid(row=4, column=1)

text_variable1 = tk.StringVar()
label_filepath = tk.Label(textvariable = text_variable1, bg = 'white', width = 50)
label_filepath.grid(row=2, column=2)
...
entry_cstring = tk.Entry(width = 70)
entry_cstring.grid(row=4, column=2)
...
button_import_file = tk.Button(window, text='Import File', command= lambda:import_file_to_db())
button_import_file.grid(row=2, column=3)

label3 = tk.Label(text= 'Choose file to import to DB')
label3.grid(row=5, column=1)

text_variable2 = tk.StringVar()
label_folderpath = tk.Label(textvariable = text_variable2, bg = 'white', width = 50)
label_folderpath.grid(row=6, column=2)
...
button_browse_folder.grid(row=6, column=1)

button_import_folder = tk.Button(window, text='Import Folder', command= lambda:import_folder_to_db())
button_import_folder.grid(row=6, column=3)
